...
handling and returning \lstinline|enum err_t| from functions.
...
\paragraph{Arithmetic operators} Operators
$+$,$-$,$*$,$/$, min\slash and max get the values from
...
For function operators, functions quote, atom, car, nth and length
are straightforward. Except quote they all evaluate the second child,
atom checks if it is not of type list, car returns its first child,
nth returns its n--th child and lenght returns field
\lstinline|node.as.list.child_count|.

\paragraph{Set}

The operator assigns a value to a variable in the
environment. It expects a list of three elements: the operator name,
the variable, and the value to assign. The function first evaluates
the variable argument; if it is a symbol and does not exist in the
environment, it is initialized. The function then ensures the
variable reference is valid, evaluates the value expression, and
makes a deep copy of the value node. The previous content of the
variable is freed and replaced with the new value. Finally, a
reference to the updated variable node is returned. All temporary
nodes are properly freed to avoid memory leaks.
