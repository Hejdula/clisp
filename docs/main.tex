...
top--down parser, so we can build the tree from root. Since we do not
need lookahead(where LL(1)\slash LL(k) parser would be usefull), it
is possible without backtracking and we can abort when we find
unbalanced parenthesis, we can use a recursive descent parser.
...
the first element is straightforward to implement as it is just the 'tail' node.
...
\begin{itemize}
  \item \textbf{Strips comments:} Removes all Lisp comments (text
    following a semicolon \lstinline|;| until the end of the line)
    from the code.
  \item \textbf{Converts to uppercase:} Changes all alphabetic
    characters to uppercase, ensuring case-insensitivity in symbol
    and operator names.
  \item \textbf{Removes line separators:} Removes line separators so
    the code is processed as a single continuous string.
\end{itemize}
...
element of the list again to determine which operator it is. So that
...
simple as we allocate and free in the same scope. For example in
\lstinline|repl| we dynamically reallocate a buffer for the input and
free it when we exit the function scope.

The issue appears during evaluation where we need to create new nodes
and return new evaluated Lisp expressions, as we want to free those
only sometimes.
For this purpose the nodes are marked with an enum \lstinline|node_origin|, see \ref{src:astnode}.

We can not simply make a copy of the returned node ensuring that we
do not free the
original tree, firstly for performance issues and its bad practice,
secondly we need to get the original node when handling variables.
For example \lstinline|(set (nth 1 arr) 1)| where \lstinline|arr| is
a variable holding list.
Due to nature of Lisp, those expressions can be complex consisting partially of
nodes coming from original AST, variables or temporary.

\paragraph{ Problematic scenario }
The expression \lstinline|(list 1 var (+ 1 2))| creates
a temporary list node with children from the original AST, variable from the
environment and a temprorary result node.
Using this expression for example in
\lstinline|(print (nth 1 (list 1 var (+ 1 2))))| % tex-fmt
we evaluate and return the content of the variable. But the temporary
result of \lstinline|(+ 1 2)| needs to be freed and neither the
\lstinline|print| or \lstinline|nth| functions access it.
