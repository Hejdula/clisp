...
child count. The \lstinline|node.type| field determines which union
member is valid. For example a first node in a list can be accessed
vith \lstinline|node.as.list.children[0]|
Lastly the origin field is used for memory management marking if the
...
Looking up the correct functions for each Lisp operator(by operators
  also refering
...
The macros simply return given value if the condition passes. The
added value over simply putting the \lstinline|return retval| into an
\lstinline|if| statement is that.
