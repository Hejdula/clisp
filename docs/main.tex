...
\title{Interpreter of Lisp Language Subset}
\author{Jan}{Hejdušek}
...
\section{Overview}
This paper is a documentation for a seminar work in the course KIV\slash
PC -- Programming in C language. I will cover the assignment itself,
analysis of the problem, details of the implementation, a short guide
...
If we treat operators the same way as variable identifiers
we can describe the structure with a simple context free grammar (CFG).
\label{fig:grammar}
\begin{align*}
...
part of our Lisp. Meaning if we parse the source code according to
this grammar, we will
miss some syntax errors that will be necessary to catch during evaluation.

We could also design a much more complex grammar that could also categorize
the operators and function calls, check the correct number of arguments and more.
With this we could find additional syntax errors in the source code,
for example that conditions expecting T or NIL
\footnote{T and NIL are special values used by Lisp to indicate
true\slash~false or an empty list}.
However most of those syntax errors could still happen at runtime due
to the nature of variables in Lisp.
Because variables can be passed almost everywhere and can contain any
valid Lisp expression,
we will have to check for those syntax errors during runtime anyway.

For our purposes, the simple grammar is sufficient enough, with the
tradeoff of how easy to parse it is. For the parsing itself, we can
use any known parser for CFG grammars. We prefer a
top--down parser, as we build the tree from the root.
...
A more complex grammar and different parsers could be better if we would like to
expand the language, but at this scale, we chose simplicity.

\section{Syntax Tree}
We need to design a data structure capable of representing the
Lisp expressions
that will later be possible to traverse and evaluate. For such
a structure there are multiple options:
\begin{enumerate}
  \item \textbf{Binary tree\slash s--expression}: One child
    represents a value of the node
    or contains another node and the other child works as a
  ...
  \item \textbf{Stack based}: The operands and operators
    could be, together
    with parentheses, pushed into a stack and retrieved for each operator.
\end{enumerate}
...
A syntax tree represented as an~S--expression has a few specific features.

Some traversals of the syntax tree are difficult to make, for example
right to left traversal requires either back--links or accumulating children.
Since every list is essentially a~linked list, access time to its
items is $O(n)$ where $n$ is the index of the item.
Also, we do not know the size of the list without going through all the nodes.

The \lstinline|NIL| object used in Lisp also appears naturally in the
...
An n--ary tree is a natural way to represent a nested parenthesized
list structure and is quite universal.
...
It outperforms s--expressions in both time and memory complexity for large lists,
with $O(1)$ access time to list items and not using as many nodes.

However, for our purpose, it is not practical exactly where
s--expressions and Lisp behave specifically.
Dynamically allocating arrays and copying data for the lists is more complex
than in s--expressions where it may be just changing one reference to a node.

\label{sec:nary-crd}
The \lstinline|CDR| function requires either having a counter of skipped children at each node
indicating whether it was shortened during evaluation and checking it on every other operation,
...
Getting a list item in \lstinline|O(1)| rather than
\lstinline|O(n)| is great, but not essential for our purpose.
These advantages outweigh the s--expression, and the few specific
behaviors of Lisp should not be difficult to implement.

\section{Evaluation}
Lisp expressions can evaluate basically to any other Lisp expression.
We may expect what the result of an evaluation is in some cases,
for example in expression \lstinline|(+ x y)| x and y should be
numbers, but mostly we do not have information about what the result will be.
Based on this, the result of an expression needs to be a generic expression.
So we will need a unified evaluation function.
...
necessary for the purposes of a simple Lisp interpreter.
...
\begin{itemize}
  ...
  \item \textbf{Parser}: Converts token array into an AST according
    to the grammar.
  \item \textbf{AST Module}: Utilities for creating, copying, and freeing AST nodes.
...
\end{itemize}
See \ref{fig:workflow} for a workflow diagram of the program.
...
See \ref{src:astnode} for the struct definition in C language.
...
Variables are stored as pairs of a string (name of the variable) and
...
An \lstinline|err_t| enum is used for several types of errors, also
including the program exit status.
Throughout the entire source code macros are widely used for error
handling and returning \lstinline|enum err_t| from functions. See
\ref{src:errt} for possible values of \lstinline|err_t| enum.
See \ref{src:macros} for a demonstration of how the macros are used.
There are currently 5 macros in use:
\begin{enumerate}
...
The first 3 macros simply return a given value, respectively NULL or an
error if the condition passes. \lstinline|CLEANUP_WITH_ERR_IF| macro
additionally jumps to a label and sets variable \lstinline|retval| to the given \lstinline|err|
...
when an error is caught, which is very helpful for the programmer.

This logging is done by the last used macro
\lstinline|LOG_IF_VERBOSE(err)| that prints the error message to the
\lstinline|stderr| stream and any subsequent calls append to the error output. If we do not want to generate all the code for logging the
...
The first argument of the program is always interpreted as a filename. It reads
...
further handled by the \lstinline|process_code_block| function.
...
passed into the \lstinline|process_code_block| function and essentially
treated in the same way as source code from a file.
The REPL continues until either
evaluation of a \lstinline|(quit)| expression or an error occurs
during evaluation and exits.

\section{Evaluation Pipeline}
Once Lisp source code is obtained, we put it through a fixed
...
\begin{itemize}
  \item Strips Lisp comments,
  \item Converts all alphabetic characters to uppercase,
  \item Removes line separators.
\end{itemize}
It does not shrink the size of the input string as it replaces removed
characters with spaces (character code 32).
These steps simplify subsequent tokenization and ensure consistent
behavior on Unix and Windows platforms.

\subsection{Lexer}
The lexer takes the normalized source code from the preprocessor and
tokenizes it. The output is a null--terminated array of C strings
(\lstinline|char**| in C).

It splits on white spaces, tabs and creates standalone one--character
tokens from characters "\lstinline|'|", "\lstinline|(|" and "\lstinline|)|".
It does not categorize the tokens.

\subsection{Parser}
For parsing we chose the grammar described above, see
\ref{fig:grammar}, and use a recursive descent parser. The parser is
...
Those functions recursively call each other and create AST nodes
corresponding to the parsed tokens and advance the current token.

The parsing is initialized with the \lstinline|parse_list| function
...
have a unified function for the evaluation:
...
a variable with this name does not exist in the environment.

For list nodes, we check the first child of the node. If it is a
symbol we compare its value (string) with all known operators using the
...
signature (except function name) with the evaluation function
\ref{fun:eval}. Their name has a prefix \lstinline|oper_| and then
keywords for the Lisp operators they implement, for example
\lstinline|oper_add|. They expect the
\lstinline|node| parameter to be a list with the operator as the first child. The
...
Most of those functions evaluate the rest of the child nodes in the list
...
replace. Then it evaluates the third argument, and
makes a deep copy of the it with origin \lstinline|VARIABLE|.
The previous content of the
variable is freed and replaced with the new node and a
...
node if it is temporary.

\paragraph{Control functions}
Function \lstinline|if| evaluates the 3rd or 4th child based on the result of
the 2nd child. 

Function \lstinline|while| cyclically evaluates all children (except
the first one) in order, and stops
if the second one is \lstinline|NIL| or the return value
...
\label{sec:mem}
Throughout the entire development, the application was tested with valgrind to check for potential memory issues and memory leaks.

For most modules and parts of evaluation, the memory management is
simple as we allocate and free in the same scope.
Tokenizing and parsing return allocated data structures and it is up
...
During evaluation, where we need to create and allocate memory for new nodes
and return new evaluated Lisp expressions, we want to free the results
only sometimes. Specifically when the node is not a variable or part
of the original AST (we need to reuse it inside \lstinline|while| loop). For this
...
temporary. Then we need to free only their temporary parts. This
partial pruning is done using the function \lstinline|free_temp_node_parts()|, which
frees the node if it is temporary and recursively frees any of its
temporary children. It does not search subtrees of non--temporary
nodes as those never contain temporary children.
...
The sample Lisp source code with bubble sort algorithm for a small (10
items) array to sort, executes as expected under a millisecond.
At around a thousand, runtime increases to 2 seconds. Maximum memory usage during the interpretation
stays almost the same in both cases under 2MB. However the number of
all allocations with 1000 items was quite large around
7~million, allocating 173MB in total.
...
was very useful and saved me many and many hours that I would have spent
debugging the code.
...
The dynamic allocating of memory for each temporary node is not
...
mentioned in the analysis \ref{sec:nary-crd}. For example \lstinline|(set (cdr arr) '(2 3))| would raise a syntax error.
...
where majority of those functions do the same with little variation.

I am not sure how the structure could be changed such that code in
those functions is less repetitive with current implementation.
It would be possible if the parsing and grammar would be
more complex and would also distinguish the different function
operators during parsing. Then we could categorize those functions
...
implementing it felt like an overkill for a project of this scale that
