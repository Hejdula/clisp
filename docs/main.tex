...
frees all parts which the struct refers to.

During evaluation, where we need to create new nodes
...
application can be built using a standard toolchain, makefiles for the GCC compiler are
provided for both GNU/Linux and Microsoft Windows environments.

\section{Execution}

The application supports two modes of operation: interactive mode and batch processing.
Executing the program without arguments initiates the interactive interpreter (REPL).
Alternatively, providing a file path as an argument processes the contents of the specified file with Lisp source code.
A verbose execution mode, which prints results of outer expressions, can be enabled by appending the \texttt{-v} flag when running with an input file.
