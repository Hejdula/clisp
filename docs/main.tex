...
A more complex grammar and other parsers could be better if we would like to
...
indicating whether it was cdr'd during evaluation and checking it on every other operation,
...
output. If we do not want to generate all the code for logging the
...
replace. Then it evaluates the value expression, and
makes a deep copy of the value node with origin \lstinline|VARIABLE|.
The previous content of the
variable is freed and replaced with the new value and a
